...
#mvn test -Dmaven.test.failure.ignore=true dependency:build-classpath -Dmdep.outputFile=cp.txt -Pwindows site:site site:deploy
...
# sed -i 's/\\/\//g' cp.txt | sed -i 's/\C:/\/cygdrive\/C/g' cp.txt | sed -i 's/\;/\:/g' cp.txt

# sed -i 's/\\/\//g' cp.txt & 
# sed -i 's/\C:/\/C/g' cp.txt &
# sed -i 's/\;/\:/g' cp.txt
